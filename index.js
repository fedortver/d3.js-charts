...
function svgBar(data, selector, settings) {
   ...
   var margin = settings.margin
         ? settings.margin
         : { top: 10, right: 0, bottom: 30, left: 0 }, //margin
      barWidth = settings.barWidth ? settings.barWidth : 13, //width bar
...
function svgTotals(data, selector, settings) {
   ...
   var margin = settings.margin
      ...
      barHeight = settings.barHeight ? settings.barHeight : 13, //width bar
   ...
   var firstRow = svg.append("g").attr("class", "g-rowTotal g-rowTotalFirst").attr( "transform", "translate(3,20)");

   var secondRow = svg.append("g").attr("class", "g-rowTotal g-rowTotalSecond").attr( "transform", "translate(3,15)");
...
function lineChart(data, dataPur, selector, settings) {
   ...
   var margin = settings.margin
         ? settings.margin
         : { left: 2, top: 80, right: 40, bottom: 110 }, // margin
      widthX = settings.width ? settings.width : 960, //width of axis x
      heightY = settings.height ? settings.height : 305, //width of axis y
      paddingFromYAxis = settings.paddingFromYAxis
         ? settings.paddingFromYAxis
         : 40, //padding from axis y
   ...
   var y = d3.scaleLinear().range([height, 0]),
      x = d3
         .scaleBand()
         .range([0, width - paddingFromYAxis])
         .paddingInner(1);
   //axis
   var xAxis = d3.axisBottom(x).tickPadding(10),
      yAxis = d3.axisLeft(y);
   ...
   var graphBlock = svg.append("g").attr("class", "graph-block");
...
function pieChart(data, selector, settings) {
   ...
   var width = settings.width ? settings.width : 400, // width
      ...
      colorRange = settings.colorRange
         ? settings.colorRange
         : d3.schemeCategory10; // array of colors for labels
...
//horizontal bar
function horBarChart(data, selector, settings) {
   if (data == undefined) return;
   if (selector == undefined) return;
   if (settings == undefined) settings = false;
   var margin = settings.margin
         ? settings.margin
         : { top: 20, right: 0, bottom: 10, left: 40 }, //margin
      heightReceipt = settings.heightReceipt ? settings.heightReceipt : 13, //height receipt bar
      heightRecieved = settings.heightRecieved ? settings.heightRecieved : 7; //height recieved bar
   (width = settings.width ? settings.width : 960), //width
      (height = settings.height ? settings.height : 230), //height
      (paddingBar = (heightReceipt - heightRecieved) / 2); //padding bar
   width = width - margin.left - margin.right;
   height = height - margin.top - margin.bottom;
   ...
   data.forEach(function(d) {
      d.details = [
         { month: d.month, name: "receipt", value: d.receipt },
         { month: d.month, name: "received", value: d.received }
      ];
   });
   ...
   graph
      ...
      .attr("y", function(d) {
         if (d.name == "received") {
            return y(d.month) - heightReceipt / 2 + paddingBar;
         }
         return y(d.month) - heightReceipt / 2;
      })
      ...
      .attr("height", function(d) {
         if (d.name == "receipt") {
            return heightReceipt;
         }
         if (d.name == "received") {
            return heightRecieved;
         }
      })
      .attr("trnasform", function(d) {
         if (d.name == "receipt") {
            return "translate(0," + paddingBar + ")";
         }
      });
}
